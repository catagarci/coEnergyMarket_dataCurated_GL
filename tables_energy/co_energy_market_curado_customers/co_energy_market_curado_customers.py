...
- JOB_NAME: co_energy_market_curado_customers
...
from extra.config.constants import (
    ...
    CAPACITY,
    PAIS,
    CATALOG_TABLE_NAME_CUSTOMERS,
    CURATED_TABLE_NAME_CUSTOMERS,
    DATA_PRODUCT_CUSTOMERS,
    NAME_CUSTOMERS,
)
...
class EnergyCustomers:
    """
    Clase para procesar los datos de los clientes.
    """
    def __init__(self, args: dict):
        # Inicializa la clase EnergyCustomers con el contexto de Glue y los parámetros del job.
        self.args = args
        ...
        self._spark: SparkSession = (
            ...
            .appName(DATA_PRODUCT_CUSTOMERS)
            .enableHiveSupport()
        ...
        self.path_raw_data = (
            ...
            f"Customers/{self.year}/{self.month}/{self.day}/{CATALOG_TABLE_NAME_CUSTOMERS}_{self.process_date}.csv"
        )

        self.path_curated_data = (
            f"s3://{PAIS}-{CAPACITY}-{DOMAIN}-curated-{self.account}-{self.env}/"
            f"table-catalog/{CURATED_TABLE_NAME_CUSTOMERS}/"
        )
    ...
    def write_to_hudi(self, raw_df):
        ...
        if not raw_df.isEmpty():
            ...
            raw_df = \
                    EnergyMarketTransform.set_cast_columns_customers(
                    raw_df
            )
            raw_df = \
                    EnergyMarketTransform.set_columns_transformation_upper(
                    raw_df,
                    column_name=NAME_CUSTOMERS
            )
            # Llamar a la función para obtener las opciones de Hudi
            hudi_options = get_hudi_options(self.catalog_database, CURATED_TABLE_NAME_CUSTOMERS)
...
if __name__ == "__main__":
    ...
    energy_customers = EnergyCustomers(args)
    energy_customers.run()
