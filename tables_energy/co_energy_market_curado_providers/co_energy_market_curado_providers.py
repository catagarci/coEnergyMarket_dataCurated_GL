...
3. Escribe los datos en formato Hudi en un entorno particionado.
"""
import sys
...
class EnergyProviders:
    ...
    def __init__(self, args: dict):
        ...
        self.path_raw_data = (
            ...
            f"{SUBDOMAIN}/Providers/{CATALOG_TABLE_NAME_PROVIDERS}_{self.process_date}.csv"
        )

        self.path_curated_data = (
            f"s3://{PAIS}-{CAPACITY}-{DOMAIN}-curated-{self.account}-{self.env}/"
            f"{SUBDOMAIN}/table-catalog/{CURATED_TABLE_NAME_PROVIDERS}/"
        )
...
if __name__ == "__main__":
    ...
    args = getResolvedOptions(sys.argv, ["JOB_NAME", "ACCOUNT", "ENV", "PROCESS_DATE", "TYPE_PROCESS"])
