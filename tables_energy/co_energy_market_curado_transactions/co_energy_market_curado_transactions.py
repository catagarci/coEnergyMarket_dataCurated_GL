...
- JOB_NAME: co_energy_market_curado_transactions
...
from extra.config.constants import (
    ...
    CAPACITY,
    PAIS,
    CATALOG_TABLE_NAME_TRANSACTIONS,
    CURATED_TABLE_NAME_TRANSACTIONS,
    DATA_PRODUCT_TRANSACTIONS,
    NAME,
    PRECIO,
)
...
class EnergyTransactions:
    """
    Clase para procesar los datos de las transacciones.
    """
    def __init__(self, args: dict):
        # Inicializa la clase EnergyTransactions con el contexto de Glue y los parámetros del job.
        self.args = args
        ...
        self._spark: SparkSession = (
            ...
            .appName(DATA_PRODUCT_TRANSACTIONS)
            .enableHiveSupport()
        ...
        self.path_raw_data = (
            ...
            f"Transactions/{self.year}/{self.month}/{self.day}/{CATALOG_TABLE_NAME_TRANSACTIONS}_{self.process_date}.csv"
        )

        self.path_curated_data = (
            f"s3://{PAIS}-{CAPACITY}-{DOMAIN}-curated-{self.account}-{self.env}/"
            f"table-catalog/{CURATED_TABLE_NAME_TRANSACTIONS}/"
        )
    ...
    def write_to_hudi(self, raw_df):
        ...
        if not raw_df.isEmpty():
            ...
            raw_df = \
                    EnergyMarketTransform.set_cast_columns_transactions(
                    raw_df
            )
            raw_df = \
            ...
            raw_df = \
                    EnergyMarketTransform.replace_nulls_with_zero(
                    raw_df,
                    column_name=PRECIO
            )
            # Llamar a la función para obtener las opciones de Hudi
            hudi_options = get_hudi_options(self.catalog_database, CURATED_TABLE_NAME_TRANSACTIONS)
...
if __name__ == "__main__":
    ...
    energy_transactions = EnergyTransactions(args)
    energy_transactions.run()
